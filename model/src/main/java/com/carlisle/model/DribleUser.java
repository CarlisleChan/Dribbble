    ...
    public DribleUser(JSONObject data) {
        try {
            ...
            if (data.has(USER_CREATED_AT)) {
                ...
                Date create_date = DateUtils.SIMPLE_DATE_FORMAT.parse(data.getString(USER_CREATED_AT));
                created_at.setTime(create_date);
            }
            if (data.has(USER_UPDATED_AT)) {
                updated_at = Calendar.getInstance();
                Date updated_date = DateUtils.SIMPLE_DATE_FORMAT.parse(data.getString(USER_UPDATED_AT));
